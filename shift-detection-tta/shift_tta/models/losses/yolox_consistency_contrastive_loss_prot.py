...
class YOLOXConsistencyContrastiveLossProt(nn.Module):
    """YOLOXConsistencyContrastiveLossProt
    Args:
    ...
    def __init__(self,
        ...
        super(YOLOXConsistencyContrastiveLossProt, self).__init__()
    ...
    def forward(self, inputs, targets, features_teacher, features_student, teacher_preds, img_width, img_height, prototypes, **kwargs):
        """Forward pass.
        Args:
        ...
        if self.contrastive:
            ...
            for i in range(len(features_teacher)): # For each feature scale calculate loss    
                ...
                ### --Use Prototypes
                if len(teacher_reg_boxes):    
                    # Find nearest prototypes based on cosine similarity
                    prototypes_scale_i = [prot[i] for prot in prototypes] # Choose the i'th scale for each class
                    cosine_distances = torch.cdist(object_features_teacher, torch.stack(prototypes_scale_i), p=2)
                    min_distances, min_indices = torch.min(cosine_distances, dim=1)
                    
                    # object_features_prototypes = [prototypes_scale_i[label] for label in teacher_label] # Choose prototype based on the pseudo-label
                    object_features_prototypes = [prototypes_scale_i[label] for label in min_indices] # Choose prototype based on min cosine distances
                
                    object_features_prototypes = torch.stack(object_features_prototypes)
                    # print(object_features_prototypes.shape)
                    object_features_all.append(object_features_prototypes) # Append object features of the prototypes
                ###                  
                ...
                if len(teacher_reg_boxes):    
                    ...
                    # print()
                    contrastive_loss = self.supconloss(object_features_all_tensor, min_indices) # Use pseudo labels as ground truths for supervision
