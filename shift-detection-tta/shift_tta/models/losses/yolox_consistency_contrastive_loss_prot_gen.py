...
class YOLOXConsistencyContrastiveLossProtGen(nn.Module):
    """YOLOXContrastiveConsistencyLossProt
    Args:
    ...
    def __init__(self,
        ...
        super(YOLOXConsistencyContrastiveLossProtGen, self).__init__()
    ...
    def forward(self, inputs, targets, features_teacher, features_student, teacher_preds, img_width, img_height, prototypes, prototypes_count, **kwargs):
        """Forward pass.
        Args:
        ...
        if self.contrastive:
            ...
            # Get GT labels
            teacher_reg_boxes = teacher_preds[0].bboxes.to(device)
            teacher_label = teacher_preds[0].labels.to(device)
            ...
            for i in range(len(features_teacher)): # For each feature scale calculate loss    
                ...
                if len(teacher_reg_boxes):      
                    ...
                    # print(object_features_teacher.shape) [N, feature_size]
                    
                    ### --Prototypes
                    for feature, label in zip(object_features_teacher, teacher_label):
                        label_index = label.item()
                        prototypes[label_index][i] += feature
                        if i == 0:
                            prototypes_count[label_index] += 1
                    ###
        ...
        return loss, consistency_loss, contrastive_loss, prototypes, prototypes_count
